...
from constraint_aware_neural_networks.models.euler.constraint import (
    Euler_Constraint_Loss,
    Euler_Constraint_Resolving_Layer,
    compute_euler_constraint_deviation,
)
from constraint_aware_neural_networks.models.model_types import Model_Type


def setup_euler_model_configuration(
    data_directory: Path,
    ...
    """
    Sets everything up for the training of the Euler model.

    .. note::
        Modify the values in this function if you want to adapt something.
    """
    feature_names = ["rhol", "ml", "El", "rhor", "mr", "Er"]
    label_names = ["rhol_s", "ml_s", "El_s", "rhor_s", "mr_s", "Er_s", "s"]

    parameters = NN_Parameters(
        algorithm_name=algorithm_type,
        model_name=Model_Type.EULER.value,
        ...
        constraint_deviation_function=compute_euler_constraint_deviation,
        constraint_loss_function=Euler_Constraint_Loss(),
        constraint_resolving_layer=Euler_Constraint_Resolving_Layer(),
        net_topology=[70] * 7,
        weight_penalty=1.0e-9,
    )
